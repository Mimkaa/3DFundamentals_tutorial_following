...
{
	// calculate steps along edges(triangle and texture)
	// (dVertex/dy)
	...
	DrawFlatTriangleTex(v0, v1, v2, texture, dv0, dv1, itEdge1);
	...
	const TexVertex& dv0, const TexVertex& dv1, TexVertex& itEdge1)
{
	...
	for (int y = yStart; y < yEnd; y++, itEdge0 += dv0, itEdge1 += dv1)
	{
		...
		const Vec2 dtcLine = (itEdge1.tc_pos - itEdge0.tc_pos) / (itEdge1.pos.x - itEdge0.pos.x);
		// create scanline tex coord interpolant and prestep
		Vec2 itcLine = itEdge0.tc_pos + dtcLine * (float(xStart) + 0.5f - itEdge0.pos.x);
