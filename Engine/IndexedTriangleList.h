...
struct IndexedTriangleList
{
	...
	static IndexedTriangleList<T> GeneratedNormals(const std::string& filename)
	{
		IndexedTriangleList<T> tl = Load(filename);
		for (auto& v : tl.vertices)
		{
			v.n = Vec3{ 0,0,0 };
		}
		for (int j = 0; j < tl.indices.size() / 3; j++)
		{
			const int i0 = tl.indices[j*3 + 0];
			const int i1 = tl.indices[j*3 + 1];
			const int i2 = tl.indices[j*3 + 2];
			T& v0 = tl.vertices[i0];
			T& v1 = tl.vertices[i1];
			T& v2 = tl.vertices[i2];
